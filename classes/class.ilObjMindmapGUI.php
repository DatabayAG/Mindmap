...
class ilObjMindmapGUI extends ilObjectPluginGUI {
    ...
    function showContent() {
        ...
        if (!file_exists($this->dataDir)) {
            ilUtil::makeDirParents($this->dataDir);
        }
        ...
        if ($data == "") {
            $data = '{"edges": [], "nodes": {"root": {"id": "root", "title":"Mindmap", "x":0, "y":0}}}';
        }
        ...
        foreach ($D['nodes'] as $key => $node) {
            $lernstand = "";
            if ($node["linktype"] == "intern" || $node["linktype"] == "lernstand") {
                ...
                if($node["linktype"] == "lernstand") {
                    $query = "SELECT
                               ud.usr_id, ud.login, ud.firstname, ud.lastname,
                               od.obj_id, objr.ref_id, od.type, od.title,
                               ulm.status status_raw,
                               (
                                   CASE
                                       WHEN ulm.status = 2 THEN 'completed'
                                       WHEN ulm.status = 1 THEN 'in_progress'
                                       WHEN ulm.status = 3 THEN 'failed'
                                       ELSE 'not_attempted'
                                   END
                               ) status,
                               ulm.status_changed
                        ...
                        INNER JOIN ut_lp_marks ulm ON ulm.obj_id = od.obj_id
                        INNER JOIN usr_data ud ON ud.usr_id = ulm.usr_id
                        WHERE objr.ref_id = '" . (int)$node['linktarget'] . "' AND ud.usr_id = '" . $ilUser->id . "' ";

                    #echo($query);
                    $res = $ilDB->query($query);
                    $anz = 0;

                    $lernstand = "not_attempted";
                    while ($row = $res->fetchRow(ilDBConstants::FETCHMODE_OBJECT)) {
                        $anz++;
                        $lernstand = $row->status;
                        break;
                    }
            ...
            $D['nodes'][$key]["lernstand"] = $lernstand;
        }
        $data = json_encode($D);
